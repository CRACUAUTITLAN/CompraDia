...
try:
    ...
    st.error("⚠️ Error configurando Google Drive. Revisa tus secretos (.streamlit/secrets.toml).")
    st.stop()
...
def buscar_o_crear_carpeta(nombre_carpeta, parent_id):
    ...
    if files:
        return files[0]['id']  # Ya existe, retornamos su ID
    else:
...
def subir_excel_a_drive(buffer, nombre_archivo):
    """Maneja la lógica de carpetas Año/Mes y sube el archivo."""
    try:
        fecha_hoy = datetime.datetime.now()
        anio = str(fecha_hoy.year)
        mes_nombre = fecha_hoy.strftime("%B")  # Nombre del mes (Ej: January)
        # Diccionario simple para meses en español si prefieres
        meses_es = {
        ...
        # 1. Verificar/Crear Carpeta AÑO dentro de la Maestra
        id_anio = buscar_o_crear_carpeta(anio, PARENT_FOLDER_ID)
        
        # 2. Verificar/Crear Carpeta MES dentro de la del Año
...
# (Las mismas funciones de limpieza de antes)
        ...
        with st.spinner('Analizando datos y conectando con Google Drive...'):
            ...
            if df_base_cuauti is not None and df_base_tulti is not None:
                ...
                if df_inv_cuauti_clean is not None and df_inv_tulti_clean is not None:
                    ...
                    # 2. LOGICA DE NEGOCIO (IGUAL QUE ANTES)
                    # --- CUAUTITLAN ---
                    ...
                    if link_drive:
                        st.success(f"✅ ¡Éxito! Archivo guardado correctamente.")
                        st.markdown(f"### [📂 Abrir archivo en Google Drive]({link_drive})")
                        st.balloons()
                    else:
                        st.error("Hubo un problema al subir a Drive.")
    else:
